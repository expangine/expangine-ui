...
import { Exprs, AnyOps } from 'expangine-runtime';
import { DEFAULT_SLOT } from '../constants';


export type IfMode = 'hide' | 'detach' | 'destroy';
...
export const CompilerIf: NodeCompiler = (template, component, scope, parent) => 
{
  ...
  if (attrs && attrs.cases)
  {
    const mode: IfMode = attrs.mode || 'detach';

    const expr = Exprs.switch(Exprs.const(true), AnyOps.isEqual);
  ...
  {
    throw new Error(`The :if directive requires a cases attribute.`);
  }
