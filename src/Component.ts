...
export type ExpressionProvider = Expression | ((c: ComponentInstanceAny) => Expression);
...
export type Component<A = never, E = never, S extends string = never, L = never, C = never> = 
  ...
  NeverPartial<L, { state: { [V in keyof L]: ExpressionProvider } }> & 
  NeverPartial<C, { computed: { [V in keyof C]: ExpressionProvider } }> & 
